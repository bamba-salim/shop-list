...
<head>
  ...
  <script type="text/javascript" src="./app/component/js/js-login.js?vr=11"></script>
  <script>
      _log_redirect();
  </script>
    ...
    <div class="px-3 py-2 min-w-full">
      <form name="form" ng-submit="logUser()">
        <div class="my-2">
          ...
          <input type="text" name="username" id="newUsername" placeholder="Identifiant" ng-model="inputs.username" class="<?= CSS::FORM_INPUT_CLASS ?>" required/>
        </div>
        <div class="my-2">
          <label for="newPassword">Mot de passe</label>
          <input type="password" name="password" id="newPassword" placeholder="Mot de passe" ng-model="inputs.password" class="<?= CSS::FORM_INPUT_CLASS ?>" required/>
        </div>
        <div class="my-2" ng-if="isNew">
        ...
        <div class="my-2">
          <input type="submit" value="{{submitButton}}" class="<?= CSS::FORM_INPUT_BTN ?>"/>
        </div>
      </form>
